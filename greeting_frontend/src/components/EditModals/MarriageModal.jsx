...
const MarriageModal = ({ data, onClose, onFetch }) => {
	...
	const handleSave = async (e) => {
		...
		try {
			...
			onClose();
			onFetch();
			toast.success("Marriage Data Updated..!", {
				position: "top-center",
				theme: "colored",
			});
		} catch (error) {
			console.error("Error updating marriage data:", error);
			const errorMessage = error.response?.data?.error || "Failed to Update marriage data";
					...
					<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
						<div className="bg-white lg:text-base text-sm rounded-lg lg:w-2/5 w-4/5 p-6 shadow-lg">
							...
							<p className="mb-6">
								...
								<span className="font-semibold">husband_name, wife_name, email, contact, marriagedate</span>
							</p>
